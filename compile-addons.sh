...
#        -v "${PWD}/${1}/src/${KVER}":/input andatoshiki/syno-toolkit:${PLATFORM}-${TOOLKIT_VER} compile-module
      docker run --rm -t --user `id -u` -v "${TMP_PATH}/${1}-mods":/output \
        -v "${PWD}/${1}/src/${KVER}":/input andatoshiki/shiki-compiler:${TOOLKIT_VER} compile-module ${PLATFORM}
